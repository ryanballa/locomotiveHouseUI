...
app.post("/api/addresses/", checkAuth, checkUserPermission, async (c) => {
  ...
  const newAddresses = await addressesModel.createAddress(
    db,
    data as addressesModel.Address
  );
...
app.put("/api/addresses/:id", checkAuth, async (c) => {
  ...
  try {
    ...
    const data = await c.req.json();
    const updatedAddress = await addressesModel.updateAddress(
      db,
      id,
      data as addressesModel.Address
    );
...
app.put("/api/clubs/:id", checkAuth, async (c) => {
  ...
  try {
    const id = c.req.param("id");
    const data = await c.req.json();
    const updatedClub = await clubsModel.updateClub(
      db,
      id,
      data as clubsModel.Club
    );
...
app.post("/api/clubs/assignments/", checkAuth, async (c) => {
  const db = dbInitalizer({ c });
  const data = await c.req.json();
  const newClubAssignments = await clubsModel.createClubAssignments(
    db,
    data as clubsModel.ClubAssignment
  );
...
app.post("/api/consists/", checkAuth, checkUserPermission, async (c) => {
  const db = dbInitalizer({ c });
  const data = await c.req.json();
  const newConsist = await consistsModel.createConsist(
    db,
    data as consistsModel.Consist
  );
...
app.post("/api/users/register", checkAuth, async (c) => {
  const { CLERK_PRIVATE_KEY } = env<{ CLERK_PRIVATE_KEY: string }>(
    c,
    "workerd"
  );
  const db = dbInitalizer({ c });
  ...
  try {
    ...
    const newUser = await usersModel.createUser(
      db,
      formattedData as usersModel.User
    );
...
app.post("/api/appointments/", checkAuth, checkUserPermission, async (c) => {
  ...
  const newAppointment = await appointmentsModel.createAppointment(
    db,
    formattedData as appointmentsModel.Appointment
  );
...
app.put("/api/appointments/:id", checkAuth, checkUserPermission, async (c) => {
  ...
  console.log(
    "Update appointment endpoint hit, id:",
    id,
    "data:",
    JSON.stringify(data)
  );
  ...
  const updatedAppointment = await appointmentsModel.updateAppointment(
    db,
    id,
    data as appointmentsModel.Appointment
  );
  ...
  if (updatedAppointment.error) {
    ...
    return c.json(
      {
        error:
          typeof updatedAppointment.error === "string"
            ? updatedAppointment.error
            : JSON.stringify(updatedAppointment.error),
      },
      400
    );
  }
...
app.delete(
  ...
  async (c) => {
    ...
    const deletedAppointment = await appointmentsModel.deleteAppointment(
      db,
      id
    );
