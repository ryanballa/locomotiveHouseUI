...
export default function Home() {
  ...
  const nextFridays = useMemo(() => {
    ...
    current.setDate(
      current.getDate() + (daysUntilFriday === 0 ? 7 : daysUntilFriday)
    );
  ...
  const fridayEveningStats = useMemo(() => {
    ...
    nextFridays.forEach((friday) => {
      ...
      const userHasAppointment = eveningAppointments.some(
        (apt) => apt.user_id === currentUserLhId
      );
  ...
  const handleFridaySignup = async (dateString: string) => {
    ...
    try {
      ...
      // Create appointment at 7 PM on the selected Friday
      // Parse the date string and set time to 7 PM in local timezone
      const appointmentDate = new Date(`${dateString}T19:00:00`);
