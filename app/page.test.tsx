...
describe('Home (Appointments Page)', () => {
  ...
  describe('When user is not signed in', () => {
    beforeEach(() => {
      ...
      (apiClient.getAppointments as any).mockRejectedValue(new Error('Unauthenticated'));
      (apiClient.getUsers as any).mockRejectedValue(new Error('Unauthenticated'));
    });
    ...
    it('should display user-friendly error message instead of "Unauthenticated"', async () => {
      ...
      expect(screen.getByText('Please sign in to view and manage appointments.')).toBeInTheDocument();
    ...
    it('should not show Friday Evening Sessions section', async () => {
      ...
      expect(screen.queryByText('Friday Evening Sessions')).not.toBeInTheDocument();
    });
  });
  ...
  describe('When user is signed in', () => {
    ...
    it('should render appointments successfully', async () => {
      ...
      expect(screen.getByRole('heading', { name: 'Appointments' })).toBeInTheDocument();
      expect(screen.getByText(/view all scheduled appointments/i)).toBeInTheDocument();
    });
    ...
    it('should not show edit/delete buttons for other user appointments', async () => {
      ...
      const janeCard = screen.getByText('Club Member: Jane Smith').closest('div');
    ...
    it('should navigate to edit page when edit button is clicked', async () => {
      ...
      expect(mockPush).toHaveBeenCalledWith(expect.stringContaining('/appointments/edit/'));
    });
    ...
    it('should show error when deletion fails', async () => {
      const user = userEvent.setup();
      window.confirm = vi.fn(() => true);
      (apiClient.deleteAppointment as any).mockResolvedValue({ deleted: false });
  ...
  describe('Friday Evening Sessions', () => {
    ...
    it('should allow user to sign up for Friday evening session', async () => {
      const user = userEvent.setup();
      (apiClient.getAppointments as any).mockResolvedValue([]);
      (apiClient.createAppointment as any).mockResolvedValue({ created: true, id: 10 });
      ...
      const signupButtons = screen.getAllByRole('button', { name: /sign up for 5 pm/i });
  ...
  describe('Error handling', () => {
    ...
    it('should handle API errors gracefully', async () => {
      (apiClient.getAppointments as any).mockRejectedValue(new Error('Network error'));
    ...
    it('should handle 401 errors with friendly message', async () => {
      (apiClient.getAppointments as any).mockRejectedValue(new Error('401 Unauthorized'));
      (apiClient.getUsers as any).mockRejectedValue(new Error('401 Unauthorized'));
